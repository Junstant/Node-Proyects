...
const handleCreateYear = async (setCareers, setErrors, careerId, career) => {
  try {
    ...
    if (result.success) {
      ...
      setErrors({});
      setCareers((prev) => prev.map((career) => (career.id === careerId ? addYearToCareer(career, result.data.payload.year.year) : career)));
    }
